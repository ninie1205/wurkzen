       ...
       <ValidationObserver v-slot="{ handleSubmit }">
         <div class="form">
            ...
            <ValidationProvider name="selectedCountry" rules="required_country" v-slot="{ errors }">
               <v-select :options="countries" label="country_name" v-model="selectedCountry" :reduce="country => country.id" :clearable=false :multiple=false @input="changedCountry" :class="{'error': errors.length > 0 }"></v-select>
            </ValidationProvider>
            ...
            <button @click="handleSubmit(updateEmployee)" :class="{'loading': isLoading}">Update employee</button>
         </div>
      </ValidationObserver>
   </div>
</template>
...
export default {
   ...
   created() {
      const getCountries = () => axios.get('https://devapi.wurkzen.com/v1/countries')
      const getStates = () => axios.get('https://devapi.wurkzen.com/v1/states')

      axios.all([getCountries(), getStates()])
         ...
         .finally( () => {
            if(this.employeeID) {
               ...
               .then( (response) => {
                  ...
                  this.selectedCountry = this.countries.filter((country) => country.id == response.data.country_id)[0]
                  if (this.selectedCountry.country_name === 'United States') {
                     this.isStateDisabled = false
                     this.selectedState = this.states.filter((state) => state.id == response.data.state_id)[0]
                  }
               })
            }
         })
